...
from .models import Medical, User_manage, Ment, Profile
...
def registerUser(request):
    if request.method == 'POST':
        ...
        a = User_manage(username=username, email=email, password=password, is_patient=True)
        a.save()
...
def loginView(request):
    if request.method == 'POST':
        username = request.POST['username']
        password = request.POST['password']
        User_manage = authenticate(request, username=username, password=password)
        if User_manage is not None and User_manage.is_active:
            auth.login(request, User_manage)
            if User_manage.is_patient:
                return redirect('patient')
            elif User_manage.is_doctor:
...
def doctor_home(request):
    doctor = User_manage.objects.filter(is_doctor=True).count()
    patient = User_manage.objects.filter(is_patient=True).count()
...
def MakePredict(request):
    ...
    #For all matched specific disease symptoms in general put 1 and unmatched pu 0
    ...
    clf = joblib.load('model/decision_tree.pk1')
...
def MakeMend(request):
  ...
  if sick in disease_list:
      ...
      clf = joblib.load('model/medical_nb.pk1')
      prediction = clf.predict(test)
